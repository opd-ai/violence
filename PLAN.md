...
**Performance outlook:** Wazero's interpreter is ~1.5–3× slower than Wasmer's JIT-compiled execution for CPU-bound WASM workloads. However, the mod system's workload profile — infrequent hook invocations (`weapon.fire`, `enemy.spawn`, `level.generate`) rather than tight inner loops — means this difference is **unobservable in practice**. Mod calls are event-driven and execute outside the 16.6ms frame budget. No regression beyond 10% is expected for any real-world mod workload; a microbenchmark regression on synthetic tight-loop WASM is expected (~2×) but does not constitute a user-facing performance degradation.
...
| **Memory read/write for string passing** | Use `wazero.Memory.Read()` which returns a `[]byte` slice backed by the WASM linear memory buffer — zero-copy. Avoid `string()` conversion; use `unsafe.String()` or `unsafe.SliceData()` for read-only access | `unsafe.String(unsafe.SliceData(bytes), len(bytes))` for zero-copy string reads |
...
- [ ] **3. Port `WASMConfig` enforcement**: Map `MemoryLimitPages` via `wazero.NewRuntimeConfig().WithMemoryLimitPages(config.MemoryLimitBytes / 65536)`. Map `FuelLimit` to `context.WithTimeout(ctx, time.Duration(config.FuelLimit/1e9) * time.Second)`. Acceptance: Loading a module with >64 MB memory request fails; long-running call is cancelled.
...
- [ ] **11. Add WASM execution benchmarks**: Create `wasm_loader_bench_test.go` with benchmarks for module compilation, function call overhead, and memory read. Acceptance: `go test -bench=. ./pkg/mod/...` produces baseline numbers; function-call overhead ≤ 10% regression vs. wasmer-go on event-hook workloads (non-tight-loop).
