...
### 3. Mobile Store Publishing — iOS/Android Submission
   ...
   **Sub-tasks**:
   ...
   3.4. [x] Implement in-app purchase stubs for store compliance (cosmetic-only) — COMPLETE (2026-03-05): Implemented comprehensive IAP system in `pkg/iap/` with Provider interface, StubProvider for desktop builds, Product/Purchase types, and predefined Catalog of 8 cosmetic-only products (developer tips, player skins, weapon trails, name colors). All products marked cosmetic=true for store compliance. Includes GetProductByID, ValidateProduct helpers, and comprehensive test coverage with 12 tests validating stub provider behavior, product lookup, validation logic, and catalog integrity. All functions under 30 lines and complexity ≤10. Zero regressions confirmed via go-stats-generator diff.
