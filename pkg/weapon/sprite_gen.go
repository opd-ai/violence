...
func generateMeleeWeapon(img *image.RGBA, rng *rand.Rand, frame FrameType) {
	...
	common.FillRect(img, 50, 90, 78, 120, handleColor)

	// Guard (cross piece)
	common.FillRect(img, 40, 88, 88, 94, color.RGBA{R: 120, G: 80, B: 40, A: 255})
	...
	// Pommel
	common.FillCircle(img, 64, 122, 6, color.RGBA{R: 140, G: 90, B: 50, A: 255})
}
...
func generateHitscanWeapon(img *image.RGBA, rng *rand.Rand, frame FrameType) {
	...
	common.FillRect(img, 64, 50, 110, 58, baseColor)
	common.FillRect(img, 64, 50, 110, 52, highlightColor) // Barrel top highlight

	// Receiver (main body)
	common.FillRect(img, 45, 54, 75, 75, baseColor)
	common.FillRect(img, 45, 54, 75, 58, highlightColor)
	...
	if frame == FrameFire {
		...
		common.FillCircle(img, 110, 54, 8, flashColor)
		// Flash rays
		for i := 0; i < 6; i++ {
	...
	// Sight (front post)
	common.FillRect(img, 100, 46, 102, 50, color.RGBA{R: 80, G: 80, B: 90, A: 255})
}
...
func generateProjectileWeapon(img *image.RGBA, rng *rand.Rand, frame FrameType) {
	...
	// Large barrel (tube)
	common.FillRect(img, 50, 45, 115, 70, bodyColor)
	common.FillRect(img, 50, 45, 115, 50, color.RGBA{R: 90, G: 90, B: 100, A: 255}) // Top highlight

	// Barrel opening (circle at end)
	common.FillCircle(img, 115, 57, 12, color.RGBA{R: 20, G: 20, B: 25, A: 255})
	common.FillCircle(img, 115, 57, 10, bodyColor)

	// Grip underneath
	common.FillRect(img, 55, 70, 70, 90, color.RGBA{R: 60, G: 40, B: 30, A: 255})

	// Energy coil accent (for plasma weapons)
	for i := 0; i < 5; i++ {
		x := 60 + i*10
		common.FillCircle(img, x, 57, 3, accentColor)
	}

	// Muzzle glow (during fire)
	if frame == FrameFire {
		glowColor := color.RGBA{R: 255, G: 200, B: 100, A: 255}
		common.FillCircle(img, 115, 57, 15, color.RGBA{R: 200, G: 150, B: 80, A: 128})
		common.FillCircle(img, 115, 57, 8, glowColor)
	}
}
