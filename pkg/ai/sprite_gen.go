...
func generateFantasyGuard(img *image.RGBA, rng *rand.Rand, frame AnimFrame) {
	...
	// Legs
	common.FillRect(img, 26, leftLegY, 30, 50, armorColor)  // Left leg
	common.FillRect(img, 34, rightLegY, 38, 50, armorColor) // Right leg

	// Body (torso)
	common.FillRect(img, 24, 18, 40, 35, armorColor)

	// Arms
	common.FillRect(img, 18, leftArmY, 24, 32, armorColor)  // Left arm
	common.FillRect(img, 40, rightArmY, 46, 32, armorColor) // Right arm

	// Head
	common.FillCircle(img, 32, 12, 6, skinColor)

	// Helmet
	common.FillRect(img, 28, 8, 36, 12, helmetColor)
	common.FillRect(img, 26, 8, 38, 10, helmetColor) // Helmet brim

	// Shield (left hand)
	common.FillRect(img, 14, 24, 20, 34, color.RGBA{R: 160, G: 140, B: 100, A: 255})

	// Sword (right hand)
	if frame == AnimFrameAttack {
		common.FillRect(img, 44, 10, 46, 28, color.RGBA{R: 200, G: 200, B: 220, A: 255})
	} else {
		common.FillRect(img, 44, 26, 46, 38, color.RGBA{R: 200, G: 200, B: 220, A: 255})
	}
}
...
func generateSciFiSoldier(img *image.RGBA, rng *rand.Rand, frame AnimFrame) {
	...
	// Legs
	common.FillRect(img, 26, leftLegY, 30, 52, armorColor)
	common.FillRect(img, 34, rightLegY, 38, 52, armorColor)

	// Body
	common.FillRect(img, 24, 18, 40, 36, armorColor)
	// Chest accent
	common.FillRect(img, 28, 22, 36, 24, accentColor)

	// Arms
	common.FillRect(img, 18, 22, 24, 34, armorColor)
	common.FillRect(img, 40, 22, 46, 34, armorColor)

	// Helmet
	common.FillCircle(img, 32, 12, 7, armorColor)
	// Visor
	common.FillRect(img, 28, 10, 36, 14, visorColor)
	...
	common.FillRect(img, 18, weaponY, 46, weaponY+2, color.RGBA{R: 60, G: 60, B: 70, A: 255})

	// Shoulder pads
	common.FillRect(img, 17, 18, 24, 22, color.RGBA{R: 60, G: 80, B: 100, A: 255})
	common.FillRect(img, 40, 18, 47, 22, color.RGBA{R: 60, G: 80, B: 100, A: 255})
}
...
func generateHorrorCultist(img *image.RGBA, rng *rand.Rand, frame AnimFrame) {
	...
	for y := 50; y >= 20; y-- {
		...
		common.FillRect(img, bodyX-width/2, y, bodyX+width/2, y+1, robeColor)
	}
	...
	common.FillRect(img, 12, armY, 24, armY+2, robeColor) // Left arm
	common.FillRect(img, 40, armY, 52, armY+2, robeColor) // Right arm

	// Hands (pale)
	common.FillCircle(img, 12, armY, 2, skinColor)
	common.FillCircle(img, 52, armY, 2, skinColor)

	// Hood
	common.FillCircle(img, 32, 12, 8, robeColor)

	// Face (shadowed, only eyes visible)
	common.FillCircle(img, 30, 12, 1, color.RGBA{R: 255, G: 50, B: 50, A: 255}) // Left eye (red)
	common.FillCircle(img, 34, 12, 1, color.RGBA{R: 255, G: 50, B: 50, A: 255}) // Right eye (red)

	// Ritual dagger (in right hand, attack frame)
	if frame == AnimFrameAttack {
		common.FillRect(img, 50, armY, 56, armY+1, color.RGBA{R: 180, G: 180, B: 190, A: 255})
	}
}
...
func generateCyberpunkDrone(img *image.RGBA, rng *rand.Rand, frame AnimFrame) {
	...
	// Main body (sphere)
	common.FillCircle(img, 32, 28+hoverY, 10, bodyColor)
	...
	common.FillCircle(img, 32, 28+hoverY, eyeRadius, neonColor)

	// Weapon pods (left and right)
	common.FillRect(img, 16, 26+hoverY, 22, 30+hoverY, bodyColor)
	common.FillRect(img, 42, 26+hoverY, 48, 30+hoverY, bodyColor)

	// Weapon barrels
	common.FillCircle(img, 16, 28+hoverY, 2, color.RGBA{R: 80, G: 80, B: 90, A: 255})
	common.FillCircle(img, 48, 28+hoverY, 2, color.RGBA{R: 80, G: 80, B: 90, A: 255})

	// Cyan accent lights
	common.FillCircle(img, 32, 20+hoverY, 2, cyanColor)
	common.FillCircle(img, 32, 36+hoverY, 2, cyanColor)

	// Muzzle flash (attack frame)
	if frame == AnimFrameAttack {
		common.FillCircle(img, 16, 28+hoverY, 4, color.RGBA{R: 255, G: 200, B: 0, A: 255})
		common.FillCircle(img, 48, 28+hoverY, 4, color.RGBA{R: 255, G: 200, B: 0, A: 255})
	}

	// Propulsion glow (bottom)
	common.FillCircle(img, 32, 40+hoverY, 3, cyanColor)
}
...
func generatePostapocScavenger(img *image.RGBA, rng *rand.Rand, frame AnimFrame) {
	...
	// Legs (ragged pants)
	common.FillRect(img, 26, leftLegY, 30, 52, clothColor)
	common.FillRect(img, 34, rightLegY, 38, 52, clothColor)

	// Body (scrap armor plates)
	common.FillRect(img, 24, 20, 40, 36, scrapColor)

	// Arms
	common.FillRect(img, 18, 24, 24, 36, clothColor) // Left arm
	common.FillRect(img, 40, 24, 46, 36, scrapColor) // Right arm (armored)

	// Head (bandana/mask)
	common.FillCircle(img, 32, 12, 6, skinColor)
	common.FillRect(img, 28, 14, 36, 16, clothColor) // Bandana over mouth

	// Goggles
	common.FillCircle(img, 29, 10, 2, color.RGBA{R: 50, G: 50, B: 50, A: 255})
	common.FillCircle(img, 35, 10, 2, color.RGBA{R: 50, G: 50, B: 50, A: 255})
	...
	common.FillRect(img, 40, weaponY, 54, weaponY+2, color.RGBA{R: 70, G: 60, B: 50, A: 255})
	common.FillCircle(img, 54, weaponY+1, 2, color.RGBA{R: 90, G: 80, B: 70, A: 255})

	// Scrap metal shoulder pad
	common.FillRect(img, 40, 20, 48, 24, scrapColor)
}
